# Dosely - Play Store Listing

## App Name
Dosely - Pill & Medication Reminder

## Short Description (80 chars max)
Never miss a dose. Simple medication reminders for you and your family.

## Full Description (4000 chars max)

Dosely is not a medical device and does not diagnose, treat, cure, or prevent any medical condition. Always consult a healthcare professional for medical advice, diagnosis, or treatment.

Dosely is a beautifully designed medication reminder app that helps you and your family stay on top of medication schedules. Whether you're managing your own prescriptions, tracking vitamins, or caring for a family member's health routine — Dosely makes it simple and reliable.

💊 SIMPLE MEDICATION MANAGEMENT
• Add medications with name, dosage, form (tablet, capsule, liquid, and more)
• Set daily or specific-day schedules with custom times
• Track pill stock and get refill reminders before you run out
• Edit any medication detail at any time — no restrictions

⏰ RELIABLE REMINDERS
• Alarm-style notifications that work even on silent mode
• Take or Snooze directly from the notification
• Never miss a dose with high-priority alerts
• Smart scheduling for morning, noon, evening, and night doses

👨‍👩‍👧‍👦 FAMILY CARE
• Create separate profiles for family members
• Track medications for parents, grandparents, spouse, or children
• Switch between profiles with one tap
• Each profile has its own medications, schedules, and history

📊 TRACK YOUR PROGRESS
• Daily progress circle shows doses taken vs. scheduled
• Weekly adherence percentage on your schedule view
• Per-medication adherence stats
• Visual status for taken, skipped, and pending doses

🔒 PRIVATE & SECURE
• All data stored locally on your device — never uploaded anywhere
• No accounts, no servers, no tracking
• Optional biometric lock and passcode protection
• Your health data stays with you, always

✨ BEAUTIFUL DESIGN
• Clean, modern interface that feels premium
• Smooth animations and haptic feedback
• Easy-to-read medication cards with clear status indicators
• Designed for all ages — simple enough for everyone

FREE FEATURES:
✓ Unlimited medication reminders
✓ Family profiles
✓ Dose tracking & adherence stats
✓ Refill reminders
✓ Biometric security

PREMIUM FEATURES:
★ Ad-free experience
★ Detailed analytics & insights
★ Export medication history
★ Custom notification sounds

Dosely is built with care for people who care about their health. We believe medication management should be simple, reliable, and beautiful — not complicated or clinical.

Download Dosely today and never miss a dose again.

---

## Category
Health & Fitness

## Tags
medication reminder, pill reminder, medicine reminder, pill tracker, medication tracker, medicine alarm, dose tracker, refill reminder, pill organizer, health tracker, medication schedule, family medication tracker

## Contact Email
support@dosely.app

## Privacy Policy URL
https://haider484991.github.io/MedReminder/privacy-policy.html

---

## Screenshots Needed (create these)
1. Dashboard with daily progress + upcoming doses
2. Add Medication screen (step 1 & 2)
3. Schedule screen with weekly calendar
4. Medication Detail with adherence stats
5. Profile screen with settings
6. Onboarding welcome screen
7. Family profile switcher
8. Refill reminder banner

## Feature Graphic
1024 x 500 px — Logo + tagline "Never miss a dose" on brand-colored background
